...
import {
  ...
  GetGeetestVerificationParams,
  SetGeetestVerificationParams,
} from "./geetest";
...
export function InitVerify(params: InitVerifyParams) {
  ...
  if (params.geetest) {
    geetest = new GeetestVerify(
      Object.assign(
        { fallbackCachePrefix: params.cachePrefix + "_fallback" },
        params.geetest
      )
    );
  }
...
export async function GetVerifyParams(params: {
  type: "sms";
  user?: AV.User;
  sms: { mobilePhoneNumber?: string };
  ...
  if (params.type == "geetest") {
    ...
    if ("sms" in params) {
      ...
      if (!user && mobilePhoneNumber) {
        ...
        let phoneUser = await new AV.Query<AV.User>("_User")
          .equalTo("mobilePhoneNumber", mobilePhoneNumber)
          .first();
...
export async function SetVerify(
  params: { type: VerifyType } & SetVerifyParams
): Promise<VerifyParams> {
  ...
  if (verifyParams.type != params.type) {
    throw new Error(
      "Error SetVerify type " + verifyParams.type + " != " + params.type
    );
  }
  if (verifyParams.type == "geetest") {
    let data = params.data as SetGeetestVerificationParams;
    if (
      !data.geetest_challenge.startsWith(
        (verifyParams as VerifyGeetestParams).data.challenge
      )
    ) {
    ...
    //验证手机号
    await AV2.Cloud.verifySmsCode(
      data.smsCode,
      (verifyParams as VerifySmsParams).data.mobilePhoneNumber
    );
  }
  return verifyParams;
}
