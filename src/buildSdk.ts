...
function getReturnTypeDeclare(node: ts.MethodDeclaration) {
  return (
    (node.type as ts.NodeWithTypeArguments)?.typeArguments?.[0]?.getText() ||
    "any"
  );
}

function createCloudRunText(
  ...
  if (clientCache) {
    let clientCacheConfig = JSON.parse(
      clientCache.replace("undefined", '""')
    ) as {
      keyPath: string[][];
    ...
    let keyPath =
      `
        (${JSON.stringify(
          clientCacheConfig.keyPath
        )}.findIndex((e:string[]) => e.every(e => Object.keys(params||{}).includes(e)) && Object.keys(params||{}).every(e2 => e.includes(e2))) >= 0) ?
    ...
    if (node.parameters.length > 0) {
      ...
      return `{
                ${versionString}
                return API.${method}('${functionName}',${parameterName},undefined,true,version||undefined,${keyPath},options?.mode||"${
        clientCacheConfig.mode || "remote"
      }",${defaultCacheText},options?.onData,options?.onError,options?.onInvokeCallback) }`;
    }
    return `{
            ${versionString}
            return API.${method}('${functionName}',undefined,undefined,true,version||undefined,${keyPath},options?.mode||"${
      clientCacheConfig.mode || "remote"
    }",${defaultCacheText},options?.onData,options?.onError,options?.onInvokeCallback) }`;
  }
  if (node.parameters.length > 0) {
...
function createSdkFile(sourceFile: ts.SourceFile) {
  ...
  function skipNode(nodeStart: ts.Node, nodeEnd: ts.Node, platform: number) {
    skipText(
      nodeStart.getFullStart(),
      (nodeEnd || nodeStart).getEnd(),
      platform
    );
  }
      ...
      case ts.SyntaxKind.FunctionDeclaration:
        {
          ...
          if (
            !IsInternalName(functionDeclaration) &&
            functionDeclaration.modifiers &&
            functionDeclaration.modifiers.find(
      ...
      case ts.SyntaxKind.ImportEqualsDeclaration:
        {
          ...
          if (
            importEqualsDeclaration.moduleReference.kind ==
      ...
      case ts.SyntaxKind.MethodDeclaration:
          ...
          if (decorators) {
            ...
            for (let i = 0; i < decorators.length; ++i) {
              ...
              if (decorator.substring(0, 6) == "@Cloud") {
                ...
                let verify =
                  verifyText && (JSON.parse(verifyText) as VerifyOptions);
                let clientCache = GetJsonValueString(decorator, "clientCache");
                let versionCb: string | undefined = undefined;
                if (clientCache) {
                  [versionCb, clientCache] = GetJsonValueString(
                    clientCache,
                    "versionCb",
                    true
                  );
                }
                let clientCacheVersionParams =
                  versionCb &&
                  (versionCb.indexOf(":") >= 0
                    ? GetJsonValueString(versionCb, "", false, 0)
                    : "any");
                ...
                // for (let i = 0; i < keys.length; ++i)
                {
                  ...
                  if (
                    ...
                    skipText(
                      decorators[0].getStart(),
                      decorators[decorators.length - 1].getEnd(),
                      i
                    );
        ...
        ${
          clientCacheVersionParams
            ...
            if (needSkip) skipAllNode(node);
          } else skipAllNode(node);
        }
        break;
    }
  }
