...
const errorMsgInfoMap: MsgIdInfoMap =
  (fs.existsSync(errorMsgFile) &&
    GetMsgInfoMap(JSON.parse(fs.readFileSync(errorMsgFile, "utf8")))) ||
  {};
 ...
 */
export function SetCloudInvokeCallback(
  callback: (name: string, request: AV.Cloud.CloudFunctionRequest) => void
) {
 ...
 * @deprecated please use init
 */
export function SetCloudErrorCallback(
  callback: (error: CloudFunctionError) => any
) {
  cloudErrorCallback = callback;
}

async function CloudHookHandler(
  request: AV.Cloud.ClassHookRequest,
  handler: AV.Cloud.ClassHookFunction,
  className: string,
  actionName: string
): Promise<any> {
  try {
    ...
    var errorInfo: CloudFunctionError = {
      ...
      params:
        ...
        _.pick(request.object.toJSON(), request.object.updatedKeys),
    ...
    // } else
    // {
    while ((error as any).ikkMessage) {
      errorInfo.errorInfo = (error as any).ikkMessage;
      error = (error as any).originalError;
    }
...
function CreateHookFunction(name: string, hook = _beforeUpdate) {
  return (className: string, handler: AV.Cloud.ClassHookFunction) => {
    hook(className, (request) =>
      CloudHookHandler(request, handler, className, name)
    );
  };
}
...
AV.Cloud.define = function (
   ...
   */
  var CloudHandler = async function (
    request: AV.Cloud.CloudFunctionRequest & { params: { _api?: SDKVersion } }
  ) {
    let ip;
    try {
      // var userAgent =
    ...
    apiVersion = {
      ...
      apiVersion:
        (apiVersion && (apiVersion.apiVersion || apiVersion["api"])) ||
        UNKNOW_STATS,
      clientVersion:
        (apiVersion && (apiVersion.clientVersion || apiVersion["version"])) ||
        UNKNOW_STATS,
    };
    try {
      //@ts-ignore
      ...
      if (info) {
        ...
        if (typeof info === "string") {
          ...
          if ((info as any).error && (info as any).target) {
            errorInfo = Object.assign(info, errorInfo);
          } else {
            while ((info as any).ikkMessage) {
              errorInfo.errorInfo = (info as any).ikkMessage;
              info = (info as any).originalError;
            }
            ...
            (info as any).target && (errorInfo.target = (info as any).target);
          }
        }
        // 创建一个ikkError并记录
        // ikkError = new IkkError(errorInfo)
  ...
  if (cacheKeyConfig) {
    ...
    for (let i = 0; i < timeUnitList.length; ++i) {
      ...
      if (Array.isArray(env)) {
        env.forEach((e) => {
          let cacheKey =
            `${prefix}:cloud:${e}:${functionName}:` +
            getCacheKey(cacheKeyConfig!);
    ...
    if (Array.isArray(env)) {
      for (let e = 0; e < env.length; ++e) {
        matches.push(
          ...(await redis.keys(`${prefix}:cloud:${env[e]}:${functionName}:*`))
        );
      }
    } else {
      matches.push(
        ...(await redis.keys(`${prefix}:cloud:${env}:${functionName}:*`))
      );
    }
