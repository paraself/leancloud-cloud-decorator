...
import {
  ...
  SetListener,
  SetAfterVerify,
} from "./leancloud-cloud-decorator";
import {
  SetCloudErrorCallback,
  SetCloudInvokeCallback,
  CloudFunctionError,
} from "./cloudHandler";
...
export function init<T = undefined>(params: InitParams<T>) {
  ...
  params.cloudInvokeCallback &&
    SetCloudInvokeCallback(params.cloudInvokeCallback);
  ...
  if (verify) {
    ...
    InitVerify(
      Object.assign({ cachePrefix: params.redisPrefix + ":verify" }, verify)
    );
  }
}
