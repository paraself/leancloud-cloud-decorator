...
function createSdkInfo(platform, dir, infoDir) {
    ...
    try {
        ...
        (0, fs_1.writeFileSync)(infoDir, 'export default ' + JSON.stringify(infoJson, null, 2), 'utf-8');
...
async function compileAndPush() {
    ...
    await (0, base_1.promiseExec)(`cd ${sdkPath} && npm version minor -f`);
    ...
    await (0, base_1.promiseExec)(`npx tsc -p ${sdkPath} && npx lcc-dep ${targetPlatform}`);
}
if (base_1.platforms[targetPlatform].type == 'dart') {
    (0, releaseDartSdk_1.releaseDartSdk)({ platform: targetPlatform });
}
else {
    compileAndPush();
}
