...
if (base_1.platforms[targetPlatform].type == 'dart') {
    let packageJson = yaml_1.default.parse((0, fs_1.readFileSync)(getSdkPackagePath(targetPlatform), 'utf-8'));
    let version = packageJson.version;
    let command = `cd ${sdkPath} && git add -A && git commit -m "auto release" && git tag -a v${version} -m "${version}" && git push --follow-tags`;
    console.log(command);
    (0, base_1.promiseExec)(command);
}
else {
    (0, base_1.promiseExec)(`npm publish ${sdkPath}`);
}
